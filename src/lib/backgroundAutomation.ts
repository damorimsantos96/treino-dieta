...
import { notificationsAreEnabled, sendWaterReminderNotification } from "@/lib/notifications";
...
async function runWaterReminderAutomation(settings: AutomationSettings) {
  ...
  const [todayLog, latestWeightLog, profile, state] = await Promise.all([
    getDailyLog(now),
    getLatestWeightLog(),
    getProfile(),
    readAutomationState(),
  ...
  const consumedMl = log.water_consumed_ml ?? 0;
  ...
  if (!progress.isBehind || progress.expectedMl <= 0) {
    return;
  }
  ...
  await writeAutomationState({
    lastWaterReminderAt: now.toISOString(),
  });
}
